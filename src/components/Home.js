...
const Home = ({ category }) => {
  ...
  if (
    mousePosition.x &&
    Math.round(mousePosition.x + 5) % ACT_RESOLUTION === 0
  ) {
    targetPos2.current = {
      x:
        -(mousePosition.x - width / 2) +
        width / diifRatioGroup +
        width / offsetRatio,
      y:
        -(mousePosition.y - height / 2) +
        height / diifRatioGroup +
        height / offsetRatio,
    };
  }
  ...
  const zIndex = {
    film:
      category !== "photo" || category !== "dvd" || category !== "note" ? 3 : 1,
  ...
  return (
    <div>
      ...
      {category === "dvd" && <DvdPage zIndex={zIndex.dvd} />}
      {category === "client" && <ClientPage zIndex={zIndex.client} />}

      {category === "note" && <NotePage zIndex={zIndex.note} />}

      {category === "contact" && <ContactPage zIndex={zIndex.contact} />}
